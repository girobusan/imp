...
function Tab({ title, textTitle, key, action, active, customClass }) {
  return html` <button
    class="Tab ${active ? "active" : "faded"} ${customClass ? customClass : ""}"
...
export default function Tabbed({
  ...
  return html` <div class=${"Tabbed " + "selectedTab_" + selected}>
    <div class="tabHeader">
      <button
        class="burgerMenu"
        onClick=${menuToggler}
    ...
    return html`<${Tab}
        ...
        e.action
          ? e.action()
          : selectFn(e.index === undefined ? i : e.index)}
        />`;
  })}
      <div
        class="dropDownMenu"
      ...
      >
        ${menuItems &&
    menuItems.map((m) => html`<${MenuItem} label=${m.label} />`)}
      </div>
    </div>
    <div
      class="tabBranding"
